...
def get_g(layer):
    """
    get Gabor parameters of filters in one layer
    """
    if hasattr(layer, 'g'):
        result = layer.g
    else:
...
def get_all_gs(layer, unwrap_shared=True):
    """
    get Gabor parameters of filters in all layers
    """
    layers = get_all_layers(layer)
...
def get_all_gs_values(layer):
    gs = get_all_gs(layer)
    return [g.get_value() for g in gs]


def set_all_gs_values(layer, values):
    gs = get_all_gs(layer)
    if len(gs) != len(values):
        raise ValueError("mismatch: got %d values to set %d parameters" %
                         (len(values), len(gs)))

    for g, v in zip(gs, values):
        if g.get_value().shape != v.shape:
            raise ValueError("mismatch: parameter has shape %r but value to "
                             "set has shape %r" %
                             (g.get_value().shape, v.shape))
        else:
            g.set_value(v)
